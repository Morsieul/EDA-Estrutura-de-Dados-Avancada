...
int Tree::_size(Node *node) { // TODO
    ...
    else return _size(node->left) + _size(node->right) + 1; 
...
int Tree::_height(Node *node) { // TODO
    if(node == nullptr) return 0;
        ...
        int left_node = _height(node->left);
        int right_node = _height(node->right);
        
        if(left_node > right_node) return ( left_node + 1);
        else return (right_node + 1);
